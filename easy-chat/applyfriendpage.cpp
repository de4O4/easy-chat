    ...
    , ui(new Ui::ApplyFriendPage)
{
    ...
    loadApplyList();
    connect(TcpMgr::getintance().get() , &TcpMgr::sig_auth_rsp , this , &ApplyFriendPage::slot_auth_rsp);
}
...
{
    //添加好友申请
    auto apply_list = UserMgr::getintance()->GetApplyList();
...
void ApplyFriendPage::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    auto find_it = _unauth_items.find(uid);
    if(find_it == _unauth_items.end()){
        return;
    }
    find_it->second->ShowAddBtn(false);
}
