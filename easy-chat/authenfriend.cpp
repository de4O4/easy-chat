    ...
    ui(new Ui::AuthenFriend),_label_point(2,6)
{
    ...
    connect(ui->tip_lb, &ClickedOnceLabel::clicked, this, &AuthenFriend::SlotAddFriendLabelByClickTip);
...
void AuthenFriend::SlotAddFriendLabelByClickTip(QString text)
{
...
void AuthenFriend::SlotApplySure()
{
    ...
    //添加发送逻辑
    QJsonObject jsonobj;
    auto uid = UserMgr::getintance()->GetUid();
    jsonobj["fromuid"] = uid;
    ...
    if(ui->back_ed->text().isEmpty()){
        ...
        backname = ui->back_ed->text();
    }
    jsonobj["backname"] = backname;
    QJsonDocument doc(jsonobj);
    QByteArray bytearray = doc.toJson(QJsonDocument::Compact);
    emit TcpMgr::getintance()->sig_send_data(ReqType::ID_AUTH_FRIEND_REQ , bytearray);
    this->hide();
    deleteLater();
}
